-- Atomically increment petition signature counts on new rows in signatures.
...
BEGIN
  UPDATE petitions
  SET
    signature_count_total = COALESCE(signature_count_total, 0) + 1,
    signature_count_verified = COALESCE(signature_count_verified, 0) + CASE WHEN COALESCE(NEW.is_verified_user, false) THEN 1 ELSE 0 END
