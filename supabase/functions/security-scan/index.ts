...
const ownerNotifyEmail = Deno.env.get("OWNER_NOTIFY_EMAIL")?.trim() ?? "";
...
serve(async (req) => {
  ...
  try {
    if (!ownerNotifyEmail) {
      console.error("[security-scan] OWNER_NOTIFY_EMAIL not set — owner account checks and alert emails disabled");
    } else {
      const { data: ownerProfile } = await supabase
        .from("profiles")
        .select("role, is_blue_verified, email")
        .eq("email", ownerNotifyEmail)
        .maybeSingle();
      ...
      if (!ownerProfile) {
        ...
        await supabase.from("profiles").update({ role: "owner_admin" }).eq("email", ownerNotifyEmail);
      }
    }
    ...
    if (ownerNotifyEmail && adminAccounts) {
      const unexpectedAdmins = adminAccounts.filter((a) => (a.email || "") !== ownerNotifyEmail);
      if (unexpectedAdmins.length > 0) {
        issues.push(`CRITICAL: unexpected admin account(s): ${unexpectedAdmins.map((a) => a.email).join(", ")}`);
      }
    }
    ...
    if (issues.length > 0 && ownerNotifyEmail && Deno.env.get("RESEND_API_KEY")) {
