...
serve(async (req) => {
  ...
  try {
    ...
    const { allowed, retryAfter } = await checkRateLimit(supabaseClient, `verify-phone-otp:${ip}`, "otp-verify");
    if (!allowed) {
      return await timed(
    ...
    const stored = normOtp(String(profile.phone_otp).trim());
