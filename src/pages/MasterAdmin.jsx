...
const PANIC_OWNER_EMAIL = import.meta.env.VITE_OWNER_PANIC_EMAIL?.trim();
if (!PANIC_OWNER_EMAIL && import.meta.env.DEV) {
  console.error("VITE_OWNER_PANIC_EMAIL is not set — panic email match disabled; owner_admin role still works.");
}
...
export default function MasterAdmin() {
  ...
  const canUsePanic = user?.role === "owner_admin" || (!!PANIC_OWNER_EMAIL && user?.email === PANIC_OWNER_EMAIL);
  ...
  const { data: allUsers = [] } = useQuery({ queryKey: ["allUsers"], queryFn: () => api.entities.User.list("-created_date", 200), enabled: !!user });
  const { data: allPolls = [] } = useQuery({ queryKey: ["allPolls"], queryFn: () => api.entities.Poll.list("-created_date", 200), enabled: !!user });
  const { data: allVotes = [] } = useQuery({ queryKey: ["allVotes"], queryFn: () => api.entities.Vote.list("-created_date", 500), enabled: !!user });
  const { data: allReports = [] } = useQuery({ queryKey: ["allReports"], queryFn: () => api.entities.Report.list("-created_date", 200), enabled: !!user });
  const { data: allPetitions = [] } = useQuery({ queryKey: ["allPetitions"], queryFn: () => api.entities.Petition.list("-created_date", 200), enabled: !!user });
  const { data: verificationRequests = [] } = useQuery({ queryKey: ["verificationRequests"], queryFn: () => api.entities.VerificationRequest.list("-created_date", 200), enabled: !!user });
  const { data: charitySubmissions = [] } = useQuery({ queryKey: ["charitySubmissions"], queryFn: () => api.entities.CharitySubmission.list("-created_date", 200), enabled: !!user });
  const { data: withdrawals = [] } = useQuery({ queryKey: ["petitionWithdrawals"], queryFn: () => api.entities.PetitionWithdrawal.list("-created_date", 200), enabled: !!user });
  const { data: donations = [] } = useQuery({ queryKey: ["platformDonations"], queryFn: () => api.entities.PlatformDonation.list("-created_date", 200), enabled: !!user });
